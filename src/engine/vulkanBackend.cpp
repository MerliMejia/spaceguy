...
static void createInstance()
{
    ...
    vulkanContext.instance = vk::raii::Instance{vulkanContext.context, createInfo};
}
...
static void createSurface()
{
    ...
    if (glfwCreateWindowSurface(*vulkanContext.instance, vulkanContext.window, nullptr, &rawSurface) != VK_SUCCESS)
    {
        throw std::runtime_error("failed to create the window surface");
    }

    vulkanContext.surface = vk::raii::SurfaceKHR{vulkanContext.instance, rawSurface};
}
...
static SwapchainSupportDetails querySwapchainSupport(VkPhysicalDevice device)
{
    ...
    details.capabilities = vk::PhysicalDevice{device}.getSurfaceCapabilitiesKHR(*vulkanContext.surface);

    details.formats =
        vk::PhysicalDevice{device}.getSurfaceFormatsKHR(*vulkanContext.surface);

    details.presentModes =
        vk::PhysicalDevice{device}.getSurfacePresentModesKHR(*vulkanContext.surface);
...
static void createLogicalDevice()
{
    ...
    vulkanContext.device = vk::raii::Device{vulkanContext.physicalDevice, createInfo};

    vulkanContext.graphicsQueue = vk::raii::Queue{vulkanContext.device, indices.graphicsFamily.value(), 0};
    vulkanContext.presentQueue = vk::raii::Queue{vulkanContext.device, indices.presentFamily.value(), 0};
}
...
static void createCommandPool()
{
    ...
    vulkanContext.commandPool = vk::raii::CommandPool{vulkanContext.device, poolInfo};
}
...
static void createSwapchain()
{
    ...
    vulkanContext.swapchain = vk::raii::SwapchainKHR{vulkanContext.device, createInfo};
...
static void createImageViews()
{
    ...
    for (vk::Image image : vulkanContext.swapchainImages)
    {
        ...
        vulkanContext.swapchainImageViews.emplace_back(vulkanContext.device, createInfo);
    }
}
